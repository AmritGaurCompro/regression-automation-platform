class Api::RecordTestsController < ActionController::API
  ...
  # POST /api/record_tests/:id/vnc_url
  # Called by GitHub Actions when Cloudflare tunnel is ready
  def vnc_url
  ...
  # POST /api/record_tests/:id/script_content
  # Called by GitHub Actions after codegen finishes — saves recorded script to DB
  def script_content
    test = Test.find_by(id: params[:id])
    return render json: { error: 'Test not found' }, status: :not_found unless test
